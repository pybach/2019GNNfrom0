...
#    data.allow_pickle=True
    losses_gnn3 = np.load('task4b_losses03.npz')
    losses_gnn4 = np.load('task4b_losses03.npz')
    ...
    if flag == 0:
        ...
        plt.legend((p1[0],p2[0]),
                   ("acc, GNN3 (with %s)" % optstr[optn], "acc, GNN4 (with %s)" % optstr[optn]),
                   loc=1)
    else:
        ### 学習曲線の描画
        n = len(losses_gnn3[0])
